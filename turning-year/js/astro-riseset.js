...
(function (A) {
  ...
  function riseSet(body, jdStart, span, lat, lon, h0) {
    ...
    var dt = span / steps;
    var rise = null, set = null, maxAlt = -Infinity, minAlt = Infinity, transit = null;
    ...
    if (prevY > maxAlt) maxAlt = prevY;

    // Sharpen the transit with a short golden-section search near the sample max.
    if (transit !== null) transit = refineTransit(body, transit - dt, transit + dt, lat, lon, h0);

    return {
      rise: rise, set: set, transit: transit,
      alwaysUp: rise === null && set === null && minAlt >= 0,
      alwaysDown: rise === null && set === null && maxAlt < 0,
      maxAltitude: maxAlt + h0used,
      minAltitude: minAlt + h0used,
  ...
  function refineTransit(body, lo, hi, lat, lon, h0) {
    ...
    for (var i = 0; i < 40; i++) {
      if (altAt(body, c, lat, lon, h0) > altAt(body, d, lat, lon, h0)) { hi = d; } else { lo = c; }
